...
class SupportController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $supports = $supports->with('user')->withCount('conversations')->latest()
            ->when($request->type == 'id', fn ($q) => $q->where('id', $request->search))
            ->when($request->type == 'ticket_no', fn ($q) => $q->where('ticket_no', $request->search))
            ->paginate(20);
